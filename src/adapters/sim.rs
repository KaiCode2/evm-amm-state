...
//! live backend, or installed as a fixture for offline tests. An eager
//! [`cold_start_many`](super::AdapterRegistry::cold_start_many) pre-warms it up
//! front (see
//! [`ProtocolMetadata::quote_code_targets`](super::ProtocolMetadata::quote_code_targets))
//! so the first quote of each family runs offline rather than paying a lazy
//! `eth_getCode`. Solidly V2 and Curve self-quote against the pool itself, so
//! they need no separate quote-target warming — but two of their quote-path
//! dependencies stay a one-time lazy first-quote fetch: Solidly's `PoolFactory`
//! (the `getFee()` target, read from pool storage rather than carried in
//! metadata) and a Curve NG pool's `DELEGATECALL`ed math/views implementation.
...
///
/// The call is executed with `from = ZERO`, `commit = false` — it does not
/// commit EVM state writes, only reading the warmed snapshot. It may still warm
/// the cache's backing data through lazy storage/code fetches when the cache is
/// live-backed, so "no mutation" refers to EVM execution state, not to the cache
/// staying byte-for-byte unchanged. Used by every per-protocol
...
pub(crate) mod abi {
    ...
    sol! {
        ...
        /// The factory address lives in pool storage (not metadata), so eager
        /// cold-start does not pre-warm the factory's code — it is a one-time lazy
        /// fetch on the first Solidly quote of each fork.

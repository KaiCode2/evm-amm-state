...
- **vs reimplemented-math libraries** (e.g. `amms-rs`, protocol SDKs): closed-form
  ...
  per-quote speed for **no formula drift** (it runs the deployed contract, so the
  AMM math cannot disagree with chain) and **uniform coverage** (any pool whose
  quote entrypoint exists works, with no new math to write). Executing the real
  bytecode removes *formula* drift, not *state* staleness — a quote is still only
  as correct as the cached read-set feeding it (see the caveats below).
...
- Executing real bytecode removes AMM *formula* drift, but a quote is still only
  as correct as the state feeding it: the cache must hold the current quote
  read-set at the intended block, with correct code and a resolved quote target.
  Stale, missing, or lazily-fetched slots and reorg lag are correctness concerns
  the benchmark's warm, pinned-block setup deliberately excludes.

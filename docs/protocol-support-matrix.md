...
| **Slipstream / Aerodrome CL** (`slipstream`) | resumable extended-surface warm-up; the incomplete canonical one-shot path is disabled | ✅ with a compatible Slipstream quoter configured and warmed | `Swap` exactness **unsupported**: routed events invalidate and repair even when research fee evidence is present; canonical four-word tick semantics are not reused | ✅ tickSpacing-keyed `getPool[t0][t1][spacing]` | deployed Base/OP runtimes additionally mutate gauge/reward/staked-liquidity state and six-word tick records; current historical/generated research is intentionally non-authoritative and lacks the exhaustive family matrix required for `Exact` |

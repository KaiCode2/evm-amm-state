...
| **Slipstream / Aerodrome CL** (`slipstream`) | as Uniswap V3 (Slipstream slot layout) | ✅ with a compatible Slipstream quoter configured and warmed | as Uniswap V3 | ✅ tickSpacing-keyed `getPool[t0][t1][spacing]` | native quote calldata uses signed `int24 tickSpacing`; the preset deliberately does not pin a chain-specific quoter address |

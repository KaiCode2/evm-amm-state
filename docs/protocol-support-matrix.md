# Protocol support matrix (v0.3.0-alpha.5)
...
| **Slipstream / Aerodrome CL** (`slipstream`) | full-range layout-only one-shot quote surface; resumable repair verifies the extended state surface | ✅ with a compatible Slipstream quoter configured and warmed | `Swap` exactness **unsupported**: routed events invalidate and repair even when research fee evidence is present; the quote-only four-word tick surface does not grant canonical transition semantics | ✅ tickSpacing-keyed `getPool[t0][t1][spacing]` | deployed Base/OP runtimes additionally mutate gauge/reward/staked-liquidity state and six-word tick records; current historical/generated research is intentionally non-authoritative and lacks the exhaustive family matrix required for `Exact` |
...
the reference storage transition with no RPC;
